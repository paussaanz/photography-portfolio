//HOMEPAGE SWIPER
import LifestyleHome from "/images/hi/lifestyle-13.webp";
import MusicHome from "/images/hi/music-1.webp";
import NatureHome from "/images/hi/nature-10.webp";
import PhotoshootsHome from "/images/hi/photoshoots-8.webp";
import SportsHome from "/images/hi/sports-5.webp";

//PORTFOLIO PAGE SWIPER
import LifestylePortfolio from "/images/low/swiper-portfolio-2.webp";
import MusicPortfolio from "/images/low/swiper-portfolio-3.webp";
import SportsPortfolio from "/images/low/swiper-portfolio-4.webp";
import NaturePortfolio from "/images/low/swiper-portfolio-5.webp";
import PhotoshootsPortfolio from "/images/low/swiper-portfolio-6.webp";

//PORTFOLIO PAGE PARALLAX
import PortfolioParallax5 from "/images/low/hero-portfolio-1.webp";
import PortfolioParallax4 from "/images/low/hero-portfolio-2.webp";
import PortfolioParallax1 from "/images/low/hero-portfolio-3.webp";
import PortfolioParallax2 from "/images/low/hero-portfolio-4.webp";
import PortfolioParallax6 from "/images/low/hero-portfolio-5.webp";
import PortfolioParallax3 from "/images/low/hero-portfolio-6.webp";

//EDITORIALS
import EditorialCover3 from "/images/hi/dolomites-cover.webp";
import EditorialCover1 from "/images/hi/greece-cover.webp";
import EditorialCover2 from "/images/hi/tanzania-cover.webp";

//EDITORIALS MOBILE
import EditorialCover3Mbl from "/images/cover-mbl-dolomites.jpg";
import EditorialCover1Mbl from "/images/cover-mbl-greece.jpg";
import EditorialCover2Mbl from "/images/cover-mbl-tanzania.jpg";
//EDITORIALS DETAILS
import Greece1 from "/images/hi/greece-1.webp";
import Greece10 from "/images/hi/greece-10.webp";
import Greece11 from "/images/hi/greece-11.webp";
import Greece2 from "/images/hi/greece-2.webp";
import Greece3 from "/images/hi/greece-3.webp";
import Greece4 from "/images/hi/greece-4.webp";
import Greece5 from "/images/hi/greece-5.webp";
import Greece6 from "/images/hi/greece-6.webp";
import Greece7 from "/images/hi/greece-7.webp";
import Greece8 from "/images/hi/greece-8.webp";
import Greece9 from "/images/hi/greece-9.webp";

import Tanzania1 from "/images/hi/tanzania-1.webp";
import Tanzania10 from "/images/hi/tanzania-10.webp";
import Tanzania2 from "/images/hi/tanzania-2.webp";
import Tanzania3 from "/images/hi/tanzania-3.webp";
import Tanzania4 from "/images/hi/tanzania-4.webp";
import Tanzania5 from "/images/hi/tanzania-5.webp";
import Tanzania6 from "/images/hi/tanzania-6.webp";
import Tanzania7 from "/images/hi/tanzania-7.webp";
import Tanzania8 from "/images/hi/tanzania-8.webp";
import Tanzania9 from "/images/hi/tanzania-9.webp";

import Dolomites1H from "/images/hi/dolomites-1.webp";
import Dolomites10H from "/images/hi/dolomites-10.webp";
import Dolomites11H from "/images/hi/dolomites-11.webp";
import Dolomites7H from "/images/hi/dolomites-7.webp";
import Dolomites11M from "/images/mid/dolomites-11.webp";
import Dolomites2M from "/images/mid/dolomites-2.webp";
import Dolomites3M from "/images/mid/dolomites-3.webp";
import Dolomites4M from "/images/mid/dolomites-4.webp";
import Dolomites5M from "/images/mid/dolomites-5.webp";
import Dolomites6M from "/images/mid/dolomites-6.webp";
import Dolomites8M from "/images/mid/dolomites-8.webp";
import Dolomites9M from "/images/mid/dolomites-9.webp";

//PHOTOSHOOTS
import Photoshoots1H from "/images/hi/photoshoots-1.webp";
import Photoshoots10H from "/images/hi/photoshoots-10.webp";
import Photoshoots11H from "/images/hi/photoshoots-11.webp";
import Photoshoots12H from "/images/hi/photoshoots-12.webp";
import Photoshoots13H from "/images/hi/photoshoots-13.webp";
import Photoshoots14H from "/images/hi/photoshoots-14.webp";
import Photoshoots15H from "/images/hi/photoshoots-15.webp";
import Photoshoots16H from "/images/hi/photoshoots-16.webp";
import Photoshoots17H from "/images/hi/photoshoots-17.webp";
import Photoshoots18H from "/images/hi/photoshoots-18.webp";
import Photoshoots19H from "/images/hi/photoshoots-19.webp";
import Photoshoots2H from "/images/hi/photoshoots-2.webp";
import Photoshoots20H from "/images/hi/photoshoots-20.webp";
import Photoshoots21H from "/images/hi/photoshoots-21.webp";
import Photoshoots22H from "/images/hi/photoshoots-22.webp";
import Photoshoots23H from "/images/hi/photoshoots-23.webp";
import Photoshoots24H from "/images/hi/photoshoots-24.webp";
import Photoshoots25H from "/images/hi/photoshoots-25.webp";
import Photoshoots26H from "/images/hi/photoshoots-26.webp";
import Photoshoots27H from "/images/hi/photoshoots-27.webp";
import Photoshoots28H from "/images/hi/photoshoots-28.webp";
import Photoshoots29H from "/images/hi/photoshoots-29.webp";
import Photoshoots3H from "/images/hi/photoshoots-3.webp";
import Photoshoots30H from "/images/hi/photoshoots-30.webp";
import Photoshoots31H from "/images/hi/photoshoots-31.webp";
import Photoshoots32H from "/images/hi/photoshoots-32.webp";
import Photoshoots33H from "/images/hi/photoshoots-33.webp";
import Photoshoots4H from "/images/hi/photoshoots-4.webp";
import Photoshoots5H from "/images/hi/photoshoots-5.webp";
import Photoshoots6H from "/images/hi/photoshoots-6.webp";
import Photoshoots7H from "/images/hi/photoshoots-7.webp";
import Photoshoots8H from "/images/hi/photoshoots-8.webp";
import Photoshoots9H from "/images/hi/photoshoots-9.webp";
import Photoshoots1M from "/images/mid/photoshoots-1.webp";
import Photoshoots10M from "/images/mid/photoshoots-10.webp";
import Photoshoots11M from "/images/mid/photoshoots-11.webp";
import Photoshoots12M from "/images/mid/photoshoots-12.webp";
import Photoshoots13M from "/images/mid/photoshoots-13.webp";
import Photoshoots14M from "/images/mid/photoshoots-14.webp";
import Photoshoots15M from "/images/mid/photoshoots-15.webp";
import Photoshoots16M from "/images/mid/photoshoots-16.webp";
import Photoshoots17M from "/images/mid/photoshoots-17.webp";
import Photoshoots18M from "/images/mid/photoshoots-18.webp";
import Photoshoots19M from "/images/mid/photoshoots-19.webp";
import Photoshoots2M from "/images/mid/photoshoots-2.webp";
import Photoshoots20M from "/images/mid/photoshoots-20.webp";
import Photoshoots21M from "/images/mid/photoshoots-21.webp";
import Photoshoots22M from "/images/mid/photoshoots-22.webp";
import Photoshoots23M from "/images/mid/photoshoots-23.webp";
import Photoshoots24M from "/images/mid/photoshoots-24.webp";
import Photoshoots25M from "/images/mid/photoshoots-25.webp";
import Photoshoots26M from "/images/mid/photoshoots-26.webp";
import Photoshoots27M from "/images/mid/photoshoots-27.webp";
import Photoshoots28M from "/images/mid/photoshoots-28.webp";
import Photoshoots29M from "/images/mid/photoshoots-29.webp";
import Photoshoots3M from "/images/mid/photoshoots-3.webp";
import Photoshoots30M from "/images/mid/photoshoots-30.webp";
import Photoshoots31M from "/images/mid/photoshoots-31.webp";
import Photoshoots32M from "/images/mid/photoshoots-32.webp";
import Photoshoots33M from "/images/mid/photoshoots-33.webp";
import Photoshoots4M from "/images/mid/photoshoots-4.webp";
import Photoshoots5M from "/images/mid/photoshoots-5.webp";
import Photoshoots6M from "/images/mid/photoshoots-6.webp";
import Photoshoots7M from "/images/mid/photoshoots-7.webp";
import Photoshoots8M from "/images/mid/photoshoots-8.webp";
import Photoshoots9M from "/images/mid/photoshoots-9.webp";

//MUSIC
import Music1H from "/images/hi/music-1.webp";
import Music2H from "/images/hi/music-2.webp";
import Music3H from "/images/hi/music-3.webp";
import Music4H from "/images/hi/music-4.webp";
import Music5H from "/images/hi/music-5.webp";
import Music6H from "/images/hi/music-6.webp";
import Music7H from "/images/hi/music-7.webp";
import Music8H from "/images/hi/music-8.webp";
import Music9H from "/images/hi/music-9.webp";
import Music1M from "/images/mid/music-1.webp";
import Music2M from "/images/mid/music-2.webp";
import Music3M from "/images/mid/music-3.webp";
import Music4M from "/images/mid/music-4.webp";
import Music5M from "/images/mid/music-5.webp";
import Music6M from "/images/mid/music-6.webp";
import Music7M from "/images/mid/music-7.webp";
import Music8M from "/images/mid/music-8.webp";
import Music9M from "/images/mid/music-9.webp";
// import Music10 from '/images/mid/music-10.webp';
import Music11H from "/images/hi/music-11.webp";
import Music12H from "/images/hi/music-12.webp";
import Music13H from "/images/hi/music-13.webp";
import Music14H from "/images/hi/music-14.webp";
import Music15H from "/images/hi/music-15.webp";
import Music11M from "/images/mid/music-11.webp";
import Music12M from "/images/mid/music-12.webp";
import Music13M from "/images/mid/music-13.webp";
import Music14M from "/images/mid/music-14.webp";
import Music15M from "/images/mid/music-15.webp";

//NATURE
import Nature1H from "/images/hi/nature-1.webp";
import Nature10H from "/images/hi/nature-10.webp";
import Nature11H from "/images/hi/nature-11.webp";
import Nature12H from "/images/hi/nature-12.webp";
import Nature13H from "/images/hi/nature-13.webp";
import Nature14H from "/images/hi/nature-14.webp";
import Nature15H from "/images/hi/nature-15.webp";
import Nature16H from "/images/hi/nature-16.webp";
import Nature17H from "/images/hi/nature-17.webp";
import Nature18H from "/images/hi/nature-18.webp";
import Nature19H from "/images/hi/nature-19.webp";
import Nature2H from "/images/hi/nature-2.webp";
import Nature20H from "/images/hi/nature-20.webp";
import Nature21H from "/images/hi/nature-21.webp";
import Nature22H from "/images/hi/nature-22.webp";
import Nature23H from "/images/hi/nature-23.webp";
import Nature24H from "/images/hi/nature-24.webp";
import Nature25H from "/images/hi/nature-25.webp";
import Nature26H from "/images/hi/nature-26.webp";
import Nature27H from "/images/hi/nature-27.webp";
import Nature28H from "/images/hi/nature-28.webp";
import Nature29H from "/images/hi/nature-29.webp";
import Nature3H from "/images/hi/nature-3.webp";
import Nature30H from "/images/hi/nature-30.webp";
import Nature4H from "/images/hi/nature-4.webp";
import Nature5H from "/images/hi/nature-5.webp";
import Nature6H from "/images/hi/nature-6.webp";
import Nature7H from "/images/hi/nature-7.webp";
import Nature8H from "/images/hi/nature-8.webp";
import Nature9H from "/images/hi/nature-9.webp";
import Nature1M from "/images/mid/nature-1.webp";
import Nature10M from "/images/mid/nature-10.webp";
import Nature11M from "/images/mid/nature-11.webp";
import Nature12M from "/images/mid/nature-12.webp";
import Nature13M from "/images/mid/nature-13.webp";
import Nature14M from "/images/mid/nature-14.webp";
import Nature15M from "/images/mid/nature-15.webp";
import Nature16M from "/images/mid/nature-16.webp";
import Nature17M from "/images/mid/nature-17.webp";
import Nature18M from "/images/mid/nature-18.webp";
import Nature19M from "/images/mid/nature-19.webp";
import Nature2M from "/images/mid/nature-2.webp";
import Nature20M from "/images/mid/nature-20.webp";
import Nature21M from "/images/mid/nature-21.webp";
import Nature22M from "/images/mid/nature-22.webp";
import Nature23M from "/images/mid/nature-23.webp";
import Nature24M from "/images/mid/nature-24.webp";
import Nature25M from "/images/mid/nature-25.webp";
import Nature26M from "/images/mid/nature-26.webp";
import Nature27M from "/images/mid/nature-27.webp";
import Nature28M from "/images/mid/nature-28.webp";
import Nature29M from "/images/mid/nature-29.webp";
import Nature3M from "/images/mid/nature-3.webp";
import Nature4M from "/images/mid/nature-4.webp";
import Nature5M from "/images/mid/nature-5.webp";
import Nature6M from "/images/mid/nature-6.webp";
import Nature7M from "/images/mid/nature-7.webp";
import Nature8M from "/images/mid/nature-8.webp";
import Nature9M from "/images/mid/nature-9.webp";
import NatureM from "/images/mid/nature.webp";

//LIFESTYLE
import Lifestyle1H from "/images/hi/lifestyle-1.webp";
import Lifestyle10H from "/images/hi/lifestyle-10.webp";
import Lifestyle11H from "/images/hi/lifestyle-11.webp";
import Lifestyle12H from "/images/hi/lifestyle-12.webp";
import Lifestyle13H from "/images/hi/lifestyle-13.webp";
import Lifestyle14H from "/images/hi/lifestyle-14.webp";
import Lifestyle15H from "/images/hi/lifestyle-15.webp";
import Lifestyle16H from "/images/hi/lifestyle-16.webp";
import Lifestyle17H from "/images/hi/lifestyle-17.webp";
import Lifestyle18H from "/images/hi/lifestyle-18.webp";
import Lifestyle19H from "/images/hi/lifestyle-19.webp";
import Lifestyle2H from "/images/hi/lifestyle-2.webp";
import Lifestyle20H from "/images/hi/lifestyle-20.webp";
import Lifestyle21H from "/images/hi/lifestyle-21.webp";
import Lifestyle22H from "/images/hi/lifestyle-22.webp";
import Lifestyle23H from "/images/hi/lifestyle-23.webp";
import Lifestyle24H from "/images/hi/lifestyle-24.webp";
import Lifestyle25H from "/images/hi/lifestyle-25.webp";
import Lifestyle26H from "/images/hi/lifestyle-26.webp";
import Lifestyle27H from "/images/hi/lifestyle-27.webp";
import Lifestyle29H from "/images/hi/lifestyle-29.webp";
import Lifestyle3H from "/images/hi/lifestyle-3.webp";
import Lifestyle30H from "/images/hi/lifestyle-30.webp";
import Lifestyle4H from "/images/hi/lifestyle-4.webp";
import Lifestyle5H from "/images/hi/lifestyle-5.webp";
import Lifestyle6H from "/images/hi/lifestyle-6.webp";
import Lifestyle7H from "/images/hi/lifestyle-7.webp";
import Lifestyle8H from "/images/hi/lifestyle-8.webp";
import Lifestyle9H from "/images/hi/lifestyle-9.webp";
import Lifestyle1M from "/images/mid/lifestyle-1.webp";
import Lifestyle10M from "/images/mid/lifestyle-10.webp";
import Lifestyle11M from "/images/mid/lifestyle-11.webp";
import Lifestyle12M from "/images/mid/lifestyle-12.webp";
import Lifestyle13M from "/images/mid/lifestyle-13.webp";
import Lifestyle14M from "/images/mid/lifestyle-14.webp";
import Lifestyle15M from "/images/mid/lifestyle-15.webp";
import Lifestyle16M from "/images/mid/lifestyle-16.webp";
import Lifestyle17M from "/images/mid/lifestyle-17.webp";
import Lifestyle18M from "/images/mid/lifestyle-18.webp";
import Lifestyle19M from "/images/mid/lifestyle-19.webp";
import Lifestyle2M from "/images/mid/lifestyle-2.webp";
import Lifestyle20M from "/images/mid/lifestyle-20.webp";
import Lifestyle21M from "/images/mid/lifestyle-21.webp";
import Lifestyle22M from "/images/mid/lifestyle-22.webp";
import Lifestyle23M from "/images/mid/lifestyle-23.webp";
import Lifestyle24M from "/images/mid/lifestyle-24.webp";
import Lifestyle25M from "/images/mid/lifestyle-25.webp";
import Lifestyle26M from "/images/mid/lifestyle-26.webp";
import Lifestyle27M from "/images/mid/lifestyle-27.webp";
import Lifestyle29M from "/images/mid/lifestyle-29.webp";
import Lifestyle3M from "/images/mid/lifestyle-3.webp";
import Lifestyle30M from "/images/mid/lifestyle-30.webp";
import Lifestyle4M from "/images/mid/lifestyle-4.webp";
import Lifestyle5M from "/images/mid/lifestyle-5.webp";
import Lifestyle6M from "/images/mid/lifestyle-6.webp";
import Lifestyle7M from "/images/mid/lifestyle-7.webp";
import Lifestyle8M from "/images/mid/lifestyle-8.webp";
import Lifestyle9M from "/images/mid/lifestyle-9.webp";

//SPORTS
import Sports1H from "/images/hi/sports-1.webp";
import Sports10H from "/images/hi/sports-10.webp";
import Sports11H from "/images/hi/sports-11.webp";
import Sports12H from "/images/hi/sports-12.webp";
import Sports13H from "/images/hi/sports-13.webp";
import Sports14H from "/images/hi/sports-14.webp";
import Sports15H from "/images/hi/sports-15.webp";
import Sports16H from "/images/hi/sports-16.webp";
import Sports17H from "/images/hi/sports-17.webp";
import Sports18H from "/images/hi/sports-18.webp";
import Sports2H from "/images/hi/sports-2.webp";
import Sports3H from "/images/hi/sports-3.webp";
import Sports4H from "/images/hi/sports-4.webp";
import Sports5H from "/images/hi/sports-5.webp";
import Sports6H from "/images/hi/sports-6.webp";
import Sports7H from "/images/hi/sports-7.webp";
import Sports8H from "/images/hi/sports-8.webp";
import Sports9H from "/images/hi/sports-9.webp";
import Sports1M from "/images/mid/sports-1.webp";
import Sports10M from "/images/mid/sports-10.webp";
import Sports11M from "/images/mid/sports-11.webp";
import Sports12M from "/images/mid/sports-12.webp";
import Sports13M from "/images/mid/sports-13.webp";
import Sports14M from "/images/mid/sports-14.webp";
import Sports15M from "/images/mid/sports-15.webp";
import Sports16M from "/images/mid/sports-16.webp";
import Sports17M from "/images/mid/sports-17.webp";
import Sports18M from "/images/mid/sports-18.webp";
import Sports2M from "/images/mid/sports-2.webp";
import Sports3M from "/images/mid/sports-3.webp";
import Sports4M from "/images/mid/sports-4.webp";
import Sports5M from "/images/mid/sports-5.webp";
import Sports6M from "/images/mid/sports-6.webp";
import Sports7M from "/images/mid/sports-7.webp";
import Sports8M from "/images/mid/sports-8.webp";
import Sports9M from "/images/mid/sports-9.webp";

export const homePageLoader = [
  [Nature1M, Nature2M, Nature3M, Nature4M, Nature5M],
  [Nature6M, Nature7M, Nature8M, Nature9M, Nature10M],
  [Nature11M, Nature12M, Nature30H, Nature14M, Nature15M],
  [Nature16M, Nature17M, Nature18M, Nature19M, Nature20M],
  [Nature21M, Nature22M, Nature23M, Nature24M, Nature25M],
];
export const homeSwiperImages = [
  {
    src: NatureHome,
    name: "home.swiper.portfolio.nature.title", // Clave de traducción para "Nature"
    date: 2023,
    description: "home.swiper.portfolio.nature.description", // Clave de traducción para "Earth's Palette"
    url: "/portfolio/nature",
  },
  {
    src: PhotoshootsHome,
    name: "home.swiper.portfolio.photoshoots.title", // Clave de traducción para "Photoshoots"
    date: 2023,
    description: "home.swiper.portfolio.photoshoots.description", // Clave de traducción para "Studio Visions"
    url: "/portfolio/photoshoots",
  },
  {
    src: SportsHome,
    name: "home.swiper.portfolio.sports.title", // Clave de traducción para "Sports"
    date: 2023,
    description: "home.swiper.portfolio.sports.description", // Clave de traducción para "Peak Performance"
    url: "/portfolio/sports",
  },
  {
    src: MusicHome,
    name: "home.swiper.portfolio.music.title", // Clave de traducción para "Music"
    date: 2023,
    description: "home.swiper.portfolio.music.description", // Clave de traducción para "Soundscapes"
    url: "/portfolio/music",
  },
  {
    src: LifestyleHome,
    name: "home.swiper.portfolio.lifestyle.title", // Clave de traducción para "Lifestyle"
    date: 2023,
    description: "home.swiper.portfolio.lifestyle.description", // Clave de traducción para "Everyday Stories"
    url: "/portfolio/lifestyle",
  },
];

export const editorialSwiperImages = [
  {
    src: EditorialCover1,
    name: "home.swiper.editorials.greece.title", // Clave de traducción para "Greece"
    date: "2023",
    description: "home.swiper.editorials.greece.description", // Clave de traducción para "Salt & Stone"
    url: "/editorials/greece",
  },
  {
    src: EditorialCover2,
    name: "home.swiper.editorials.tanzania.title", // Clave de traducción para "Tanzania"
    date: "2022",
    description: "home.swiper.editorials.tanzania.description", // Clave de traducción para "Raw Earth"
    url: "/editorials/tanzania",
  },
  {
    src: EditorialCover3,
    name: "home.swiper.editorials.dolomites.title", // Clave de traducción para "Dolomites"
    date: "2022",
    description: "home.swiper.editorials.dolomites.description", // Clave de traducción para "Jagged Dreams"
    url: "/editorials",
  },
];

export const portfolioParallaxHero = [
  {
    src: PortfolioParallax1,
    y: "sm",
  },
  {
    src: PortfolioParallax2,
    y: "md",
  },
  {
    src: PortfolioParallax3,
    y: "lg",
  },
  {
    src: PortfolioParallax4,
    y: "sm",
  },
  {
    src: PortfolioParallax5,
    y: "md",
  },
  {
    src: PortfolioParallax6,
    y: "lg",
  },
];

export const editorialsParallaxHero = [
  {
    src: Greece6,
    y: " md",
  },
  {
    src: Tanzania3,
    y: "md",
  },
  {
    src: Dolomites11H,
    y: "lg",
  },
  {
    src: Tanzania1,
    y: "sm",
  },
  {
    src: Dolomites7H,
    y: "sm",
  },
];

export const portfolioCardAnimation = [
  {
    src: PhotoshootsPortfolio,
    name: "portfolio.cards.photoshoots.title",
    date: "2023",
    description:
      "SYP's photoshoots blend vivid colors and dynamic lighting in studio and outdoor settings, showcasing creative posing and keen visual flair.",
    url: "/portfolio/photoshoots",
  },
  {
    src: NaturePortfolio,
    name: "portfolio.cards.nature.title",
    date: "2024",
    description:
      "Capturing the essence of the natural world, these photos focus on intricate details and vibrant landscapes in both familiar and remote settings.",
    url: "/portfolio/nature",
  },
  {
    src: SportsPortfolio,
    name: "portfolio.cards.sports.title",
    date: "2023",
    description: "Description 3",
    url: "/portfolio/sports",
  },
  {
    src: MusicPortfolio,
    name: "portfolio.cards.music.title",
    date: "2023",
    description: "Description 3",
    url: "/portfolio/music",
  },
  {
    src: LifestylePortfolio,
    name: "portfolio.cards.lifestyle.title",
    date: "2024",
    description: "Description 3",
    url: "/portfolio/lifestyle",
  },
];

export const editorialsCovers = [
  {
    src: EditorialCover1,
    name: "GREECE",
    date: "0723",
    url: "/editorials/greece",
  },
  {
    src: EditorialCover2,
    name: "TANZANIA",
    date: "0423",
    url: "/editorials/tanzania",
  },
  {
    src: EditorialCover3,
    name: "DOLOMITES",
    date: "0624",
    url: "/editorials/dolomites",
  },
];

export const editorialsDetails = {
  greece: {
    heroImage: { src: Greece7 },
    title: "Seafaring Greece",
    subtitles: ["blue horizons", "Sail style", "wave makers"],
    heroText:
      "Living on a sailing boat in Greece offers a unique and enchanting lifestyle surrounded by the breathtaking beauty of the Mediterranean. Imagine waking up to the gentle sounds of waves lapping against the hull and the soft caress of the sea breeze. Everyday life becomes a harmonious dance between the elements and the simplicity of living with the essentials. The open seas become your backyard, inviting you to explore hidden coves, secret beaches, and pristine coastlines.",
    fontSize: "8vw",
    firstSection: [{ src: Greece1 }, { src: Greece2 }, { src: Greece3 }],
    firstSectionTitle:
      "Exploring the Intriguing Blend of LIFESTYLE AND PERFORMANCE ON BOARD",
    firstSectionText:
      "This lifestyle fosters a deep connection with nature. From swimming in crystal-clear waters to sunbathing on the deck, the sea becomes your playground. Freshly caught fish, paired with local produce, create delectable meals enjoyed under the open sky. Evenings are a symphony of colors as the sunsets cast a warm golden glow over the horizon. With no fixed address, you have the freedom to sail from one idyllic island to another, discovering the rich history, vibrant culture, and charming coastal towns that Greece has to offer. Living on a sailing boat in Greece is an embodiment of freedom, simplicity, and a deep appreciation for the wonders of the natural world. It is an invitation to slow down, embrace the serenity of the sea, and create unforgettable memories against the backdrop of the stunning Greek landscape.",
    firstSectionText2:
      "Every corner of the Aegean Sea offers moments that beg to be captured through the lens. The tranquil mornings, with soft sunlight illuminating the water's surface, create a perfect setting for serene and atmospheric shots. Imagine the golden glow reflecting off the hull of the boat or the quiet elegance of the sails catching the first breeze of the day. These quiet beginnings, framed by the infinite blue of the sky and sea, set the stage for a photographer's dream.",
    secondSection: [{ src: Greece4 }, { src: Greece5 }, { src: Greece6 }],
    secondSectionText:
      "As you sail through the islands, the changing landscapes provide an endless array of compositions, each offering its own unique character and visual allure. The vibrant greens of Skiathos’ pine forests stretch toward the sky, their dense canopies creating a rich, verdant frame for the coastline, where the sparkling turquoise waters shimmer with an almost otherworldly clarity. The interplay of light and shadow on the waves creates an ever-changing mosaic, perfect for capturing dynamic seascapes that seem to breathe with life. Nearby coves reveal secluded beaches with golden sands, offering intimate vignettes of untouched beauty, while rocky headlands extend into the sea, their jagged forms providing a dramatic contrast to the soft fluidity of the water. As the journey continues to Alonissos, the scenery shifts to a rugged, untamed charm. The towering cliffs, sculpted over centuries by the relentless forces of wind and water, rise majestically above the sea, their textured faces catching the warm glow of the sun. In the golden hours of dusk, these cliffs stand silhouetted against a sky painted in pastel hues of lavender, peach, and amber, creating a breathtaking juxtaposition that captures the timeless solitude of this island. Each island reveals a new story, a fresh palette of colors and textures, from the pristine waters and lush landscapes to the interplay of light and shadow that defines the ever-changing beauty of the Aegean. These natural wonders invite the viewer not only to admire but to feel the soul of the Greek islands, where the land, sea, and sky come together in perfect harmony.",
    thirdSection: [{ src: Greece8 }, { src: Greece9 }],
    thirdSectionTitle:
      "Exploring the Breathtaking Landscapes of the Greek Islands",
    thirdSectionText: [
      "Each island in Greece unfolds its own story through its unique landscapes and enchanting charm. In Skiathos, lush pine forests cascade toward pristine shores, their vibrant greens contrasting with the crystalline turquoise waters that lap gently at the beaches. Alonissos captivates with its rugged cliffs and hidden coves, offering breathtaking views and a sense of untouched wilderness, while Skopelos showcases its iconic whitewashed chapels nestled among olive groves, blending human artistry with natural beauty.",
      "Wandering through these islands feels like stepping into a living painting. Narrow, winding paths lead to picturesque villages where the simplicity of whitewashed houses is offset by bursts of vivid bougainvillea spilling from balconies. These serene settlements invite exploration, with unexpected delights around every corner—hidden tavernas serving freshly caught seafood, cozy cafes shaded by ancient trees, and historic churches that echo with centuries of tradition. Each moment on the islands offers a new perspective on their timeless allure.",
    ],
    fourthSection: [{ src: Greece10 }, { src: Greece11 }],
  },
  tanzania: {
    heroImage: { src: Tanzania6 },
    title: "Tanzania Treasures",
    subtitles: ["Endless Plains", "Wild Majesty", "Nature's Pulse"],
    heroText:
      "Tanzania's landscapes serve as an endless muse, inspiring both awe and introspection. From the expansive plains of the Serengeti, where the golden grasslands seemingly stretch to infinity, to the ethereal Ngorongoro Crater, a world within itself, every frame tells a story of timeless beauty. Through my photographs, I aim to transport viewers to these remarkable places, allowing them to witness the serenity of a sunrise painting the savannah in hues of gold or the majesty of an elephant gracefully roaming across the horizon. Tanzania's landscapes are a testament to the power and magnificence of nature, and it is my privilege to share these moments frozen in time.",
    fontSize: "7vw",
    firstSection: [{ src: Tanzania1 }, { src: Tanzania2 }, { src: Tanzania3 }],
    firstSectionTitle:
      "Immersing Yourself in the Raw Beauty and Spirit of the Wild",
    firstSectionText:
      "Life in Tanzania is an ever-changing spectacle of nature, where every moment is a chance to connect deeply with the wild. Whether you’re gazing at a lioness prowling the savannah or marveling at the elegance of giraffes silhouetted against the horizon, the untamed beauty of this land is impossible to ignore. Safari adventures through the Serengeti offer close encounters with Africa’s most iconic wildlife, while the Great Migration—a spectacle of millions of wildebeest and zebras crossing perilous rivers—reveals the relentless rhythm of life. Beyond the game drives, Tanzania’s local culture adds richness to every journey. Visiting a Maasai village unveils a way of life deeply intertwined with the land, offering insight into their traditions and resilience. Whether it's trekking the slopes of Kilimanjaro or walking the sands of Zanzibar’s beaches, Tanzania promises an unfiltered, unforgettable connection to the wild.",
    firstSectionText2:
      "Photographers find their muse in Tanzania’s ever-shifting light and dramatic landscapes. The golden hour bathes the savannah in a warm, magical glow, casting long shadows and igniting the horizon with vibrant colors. The stark beauty of the baobab trees, standing solitary in the vast plains, and the dust trails kicked up by herds on the move provide a canvas for storytelling through the lens. Each shot captures not just a moment but an emotion—a testament to the enduring power of the natural world.",
    secondSection: [{ src: Tanzania4 }, { src: Tanzania5 }, { src: Tanzania6 }],
    secondSectionText:
      "Tanzania’s terrain is a kaleidoscope of natural wonders, each offering a distinct experience. The Ngorongoro Crater, often called the 'Eighth Wonder of the World,' is a microcosm of East Africa’s wildlife, its floor teeming with elephants, buffalo, and grazing antelope. To the north, Mount Kilimanjaro rises in quiet defiance, its glaciers gleaming against an endless blue sky. Trekking its forested slopes reveals unexpected surprises—playful monkeys swinging from branches, exotic birds flitting between trees, and an overwhelming sense of serenity as the summit comes into view. And then there’s Zanzibar, a tropical haven where white sands and crystal-clear waters invite rest and rejuvenation. Beneath the waves, coral reefs teem with vibrant marine life, while onshore, the scent of cloves and nutmeg fills the air. Every part of Tanzania feels alive, connected, and vibrant, inviting travelers to discover a place where the natural world still holds its dominion.",
    thirdSection: [{ src: Tanzania7 }, { src: Tanzania8 }],
    thirdSectionTitle: "Witnessing Tanzania's Awe-Inspiring Diversity",
    thirdSectionText: [
      "Tanzania is a land of contrasts, not only in its landscapes but in its people and traditions. In the Serengeti, you’ll find Maasai communities living in harmony with the land, their vibrant shukas (robes) a striking contrast to the earthy tones of the savannah. The rhythmic chants of their ceremonies echo ancient wisdom, connecting past and present in a way that feels timeless. Along the coastline, Zanzibar offers a cultural tapestry influenced by centuries of trade, where Persian, Arab, and African traditions converge.",
      "Every experience in Tanzania is enriched by the connection between people and place. Whether you’re learning about the delicate balance of ecosystems from a local guide, sharing a meal prepared with the day’s fresh catch, or listening to the stories of elders beneath a sky thick with stars, Tanzania feels like more than just a trip—it feels like a return to something essential, a reminder of the beauty and resilience of life.",
    ],
    fourthSection: [{ src: Tanzania9 }, { src: Tanzania10 }],
    fourthSection: [{ src: Tanzania9 }, { src: Tanzania10 }],
  },
  dolomites: {
    heroImage: { src: Dolomites1H },
    title: "Stone horizons",
    subtitles: ["towering peaks", "wild beauty", "timeless charm"],
    heroText:
      "Nestled in the heart of the Alps, the Dolomites offer an unparalleled escape into a world of rugged beauty and serene landscapes. Imagine waking to the golden glow of sunrise casting its warmth on jagged limestone peaks or wandering through lush meadows filled with wildflowers. Every day in this majestic region is an invitation to explore the harmonious blend of untamed wilderness and timeless alpine charm. From peaceful mountain lakes to dramatic cliffs, the Dolomites are a playground for adventurers and nature lovers alike.",
    fontSize: "9vw",
    firstSection: [
      { src: Dolomites2M },
      { src: Dolomites3M },
      { src: Dolomites4M },
    ],
    firstSectionTitle:
      "Embracing the Perfect Balance of Alpine ADVENTURE and TRANQUILITY",
    firstSectionText:
      "Life in the Dolomites offers a rare opportunity to live in harmony with some of the most striking landscapes on Earth. Whether you’re trekking through dense pine forests, where sunlight dances on the forest floor, or scaling sheer rock faces that challenge your endurance and skill, every moment here brings you closer to nature. Yet, the Dolomites are not just for adventurers—they’re also a haven for those seeking peace and quiet. The traditional alpine huts, known as 'rifugi,' offer more than just shelter; they are places where the warmth of a wood fire and a hearty meal, made from fresh local ingredients, create a sense of home. Signature dishes like polenta with melted cheese and speck, or freshly baked strudel, offer a taste of the region’s rich culinary heritage. As the day ends, the phenomenon of 'enrosadira'—the magical alpenglow—bathes the peaks in hues of pink and orange, creating moments of awe and reflection. This balance between exhilaration and serenity defines life in the Dolomites, making it a destination that speaks to both the body and the soul.",
    firstSectionText2:
      "For photographers and lovers of visual beauty, the Dolomites are a treasure trove of inspiration. Early mornings bring an ethereal quality to the landscape, as mist rises gently from the valleys and the first rays of sunlight illuminate the rugged peaks. Every detail here seems designed to be captured—from the mirror-like reflections of pristine lakes like Lago di Carezza and Lago di Braies, to the intricate wooden architecture of mountain villages framed by the backdrop of jagged cliffs. ",
    secondSection: [
      { src: Dolomites5M },
      { src: Dolomites6M },
      { src: Dolomites7H },
    ],
    secondSectionText:
      "The Dolomites are a region of contrasts, where every turn reveals a new and awe-inspiring landscape. In the tranquil pastures of Val Gardena, rolling green hills are dotted with traditional farms and grazing cattle, offering a picture of alpine life that feels untouched by time. Travel a little further, and the scenery becomes more dramatic, with the towering spires of Tre Cime di Lavaredo dominating the horizon. These iconic peaks, with their sharp edges and sheer cliffs, are a testament to the power and beauty of natural forces. The lakes of the Dolomites add another layer of enchantment—Lago di Braies, with its emerald-green waters, seems like a gem hidden among the mountains, while Lago di Sorapis, with its almost surreal turquoise hue, invites quiet contemplation. Every step you take in this extraordinary region offers something new—a secluded grove where sunlight filters through ancient trees, a panoramic vista that leaves you breathless, or a narrow trail that winds its way through a rocky pass, leading to yet another hidden wonder. The Dolomites are not just a destination; they are a never-ending journey into the heart of nature’s most spectacular creations.",
    thirdSection: [{ src: Dolomites8M }, { src: Dolomites9M }],
    thirdSectionTitle:
      "Revealing the Unique Character of Each Valley, Peak, and Alpine Village",
    thirdSectionText: [
      "Every corner of the Dolomites has its own distinct character and story, making this region a patchwork of unforgettable experiences. In Val di Funes, the iconic St. Magdalena church, framed by the jagged Odle peaks, captures the imagination with its perfect balance of natural and human artistry. Cortina d’Ampezzo, often called the 'Queen of the Dolomites,' blends sophistication and adventure, offering world-class skiing, hiking, and a vibrant cultural scene surrounded by some of the most photogenic summits in the Alps.",
      "Exploring the Dolomites is also a journey through time and tradition. Wooden chalets adorned with colorful flower boxes stand as proud testaments to the region’s heritage, while narrow village streets lead to bustling markets where the scents of freshly baked bread and aged cheese fill the air. The sound of cowbells echoes through the valleys, a reminder of the enduring connection between the people and their land. Historic churches, often perched on hilltops or hidden within the folds of the landscape, provide moments of quiet reflection and awe.",
    ],
    fourthSection: [{ src: Dolomites10H }, { src: Dolomites11M }],
  },
};

export const portfolioDetails = {
  photoshoots: {
    heroImage: {
      mobileSrc: Photoshoots16M,
      src: Photoshoots8H,
    },
    projectImages: [
      {
        srcM: Photoshoots1M,
        srcH: Photoshoots1H,
        colStart: 1,
        colSpan: 4,
        rowStart: 1,
        rowSpan: 6,
        size: "100%",
        id: "1-0",
      },
      {
        srcM: Photoshoots2M,
        srcH: Photoshoots2H,
        colStart: 8,
        colSpan: 4,
        rowStart: 1,
        rowSpan: 6,
        height: "70%",
        width: "85%",
        id: "1-1",
      },
      {
        srcM: Photoshoots3M,
        srcH: Photoshoots3H,
        colStart: 3,
        colSpan: 4,
        rowStart: 7,
        rowSpan: 7,
        height: "85%",
        width: "100%",
        id: "1-2",
      },
      {
        srcM: Photoshoots4M,
        srcH: Photoshoots4H,
        colStart: 8,
        colSpan: 4,
        rowStart: 7,
        rowSpan: 6,
        id: "1-3",
      },
      {
        srcM: Photoshoots5M,
        srcH: Photoshoots5H,
        colStart: 2,
        colSpan: 6,
        rowStart: 14,
        rowSpan: 5,
        id: "1-4",
      },
      {
        srcM: Photoshoots6M,
        srcH: Photoshoots6H,
        colStart: 9,
        colSpan: 4,
        rowStart: 15,
        rowSpan: 7,
        id: "2-0",
      },
      {
        srcM: Photoshoots7M,
        srcH: Photoshoots7H,
        colStart: 1,
        colSpan: 4,
        rowStart: 20,
        rowSpan: 6,
        id: "2-1",
      },
      {
        srcM: Photoshoots8M,
        srcH: Photoshoots8H,
        colStart: 6,
        colSpan: 6,
        rowStart: 23,
        rowSpan: 4,
        id: "2-2",
      },
      {
        srcM: Photoshoots9M,
        srcH: Photoshoots9H,
        colStart: 3,
        colSpan: 4,
        rowStart: 28,
        rowSpan: 6,
        id: "2-3",
      },
      {
        srcM: Photoshoots10M,
        srcH: Photoshoots10H,
        colStart: 8,
        colSpan: 4,
        rowStart: 29,
        rowSpan: 3,
        id: "2-4",
      },
      {
        srcM: Photoshoots11M,
        srcH: Photoshoots11H,
        colStart: 8,
        colSpan: 4,
        rowStart: 34,
        rowSpan: 6,
        id: "3-0",
      },
      {
        srcM: Photoshoots12M,
        srcH: Photoshoots12H,
        colStart: 2,
        colSpan: 4,
        rowStart: 35,
        rowSpan: 5,
        id: "3-1",
      },
      {
        srcM: Photoshoots13M,
        srcH: Photoshoots13H,
        colStart: 1,
        colSpan: 6,
        rowStart: 41,
        rowSpan: 9,
        id: "3-2",
      },
      {
        srcM: Photoshoots14M,
        srcH: Photoshoots14H,
        colStart: 9,
        colSpan: 3,
        rowStart: 41,
        rowSpan: 4,
        id: "3-3",
      },
      {
        srcM: Photoshoots15M,
        srcH: Photoshoots15H,
        colStart: 8,
        colSpan: 5,
        rowStart: 47,
        rowSpan: 10,
        id: "3-4",
      },
      {
        srcM: Photoshoots16M,
        srcH: Photoshoots16H,
        colStart: 3,
        colSpan: 3,
        rowStart: 51,
        rowSpan: 5,
        id: "4-0",
      },
      {
        srcM: Photoshoots17M,
        srcH: Photoshoots17H,
        colStart: 9,
        colSpan: 3,
        rowStart: 58,
        rowSpan: 5,
        id: "4-1",
      },
      {
        srcM: Photoshoots18M,
        srcH: Photoshoots18H,
        colStart: 2,
        colSpan: 4,
        rowStart: 57,
        rowSpan: 5,
        id: "4-2",
      },
      {
        srcM: Photoshoots19M,
        srcH: Photoshoots19H,
        colStart: 3,
        colSpan: 3,
        rowStart: 63,
        rowSpan: 5,
        id: "4-3",
      },
      {
        srcM: Photoshoots20M,
        srcH: Photoshoots20H,
        colStart: 7,
        colSpan: 3,
        rowStart: 64,
        rowSpan: 6,
        id: "4-4",
      },
      {
        srcM: Photoshoots21M,
        srcH: Photoshoots21H,
        colStart: 4,
        colSpan: 4,
        rowStart: 69,
        rowSpan: 5,
        id: "5-0",
      },
      {
        srcM: Photoshoots22M,
        srcH: Photoshoots22H,
        colStart: 9,
        colSpan: 4,
        rowStart: 71,
        rowSpan: 6,
        id: "5-1",
      },
      {
        srcM: Photoshoots23M,
        srcH: Photoshoots23H,
        colStart: 2,
        colSpan: 5,
        rowStart: 75,
        rowSpan: 3,
        id: "5-2",
      },
      {
        srcM: Photoshoots24M,
        srcH: Photoshoots24H,
        colStart: 8,
        colSpan: 4,
        rowStart: 78,
        rowSpan: 5,
        id: "5-3",
      },
      {
        srcM: Photoshoots25M,
        srcH: Photoshoots25H,
        colStart: 2,
        colSpan: 3,
        rowStart: 79,
        rowSpan: 4,
        id: "5-4",
      },
      {
        srcM: Photoshoots26M,
        srcH: Photoshoots26H,
        colStart: 4,
        colSpan: 3,
        rowStart: 82,
        rowSpan: 4,
        id: "6-0",
      },
      {
        srcM: Photoshoots27M,
        srcH: Photoshoots27H,
        colStart: 2,
        colSpan: 4,
        rowStart: 87,
        rowSpan: 3,
        id: "6-1",
      },
      {
        srcM: Photoshoots28M,
        srcH: Photoshoots28H,
        colStart: 8,
        colSpan: 3,
        rowStart: 84,
        rowSpan: 5,
        id: "6-2",
      },
      {
        srcM: Photoshoots29M,
        srcH: Photoshoots29H,
        colStart: 2,
        colSpan: 10,
        rowStart: 91,
        rowSpan: 5,
        id: "6-3",
      },
      {
        srcM: Photoshoots30M,
        srcH: Photoshoots30H,
        colStart: 2,
        colSpan: 3,
        rowStart: 97,
        rowSpan: 4,
        id: "6-4",
      },
      {
        srcM: Photoshoots31M,
        srcH: Photoshoots31H,
        colStart: 8,
        colSpan: 5,
        rowStart: 97,
        rowSpan: 8,
        id: "7-0",
      },
      {
        srcM: Photoshoots32M,
        srcH: Photoshoots32H,
        colStart: 2,
        colSpan: 5,
        rowStart: 102,
        rowSpan: 3,
        id: "7-1",
      },
      {
        srcM: Photoshoots33M,
        srcH: Photoshoots33H,
        colStart: 1,
        colSpan: 4,
        rowStart: 106,
        rowSpan: 7,
        id: "7-2",
      },
    ],
  },
  music: {
    heroImage: {
      mobileSrc: Music11M,
      src: Music11H,
    },
    projectImages: [
      {
        srcM: Music1M,
        srcH: Music1H,
        colStart: 1,
        colSpan: 4,
        rowStart: 1,
        rowSpan: 8,
        id: "1-0",
      },
      {
        srcM: Music2M,
        srcH: Music2H,
        colStart: 7,
        colSpan: 3,
        rowStart: 2,
        rowSpan: 6,
        id: "1-1",
      },
      {
        srcM: Music3M,
        srcH: Music3H,
        colStart: 6,
        colSpan: 6,
        rowStart: 9,
        rowSpan: 4,
        id: "1-2",
      },
      {
        srcM: Music4M,
        srcH: Music4H,
        colStart: 2,
        colSpan: 4,
        rowStart: 12,
        rowSpan: 6,
        width: "90%",
        id: "1-3",
      },
      {
        srcM: Music5M,
        srcH: Music5H,
        colStart: 8,
        colSpan: 3,
        rowStart: 14,
        rowSpan: 4,
        id: "1-4",
      },
      {
        srcM: Music6M,
        srcH: Music6H,
        colStart: 7,
        colSpan: 6,
        rowStart: 19,
        rowSpan: 7,
        width: "90%",
        id: "2-0",
      },
      {
        srcM: Music7M,
        srcH: Music7H,
        colStart: 2,
        colSpan: 5,
        rowStart: 25,
        rowSpan: 5,
        width: "95%",
        id: "2-1",
      },
      {
        srcM: Music8M,
        srcH: Music8H,
        colStart: 1,
        colSpan: 3,
        rowStart: 19,
        rowSpan: 5,
        width: "90%",
        id: "2-2",
      },
      {
        srcM: Music9M,
        srcH: Music9H,
        colStart: 8,
        colSpan: 4,
        rowStart: 27,
        rowSpan: 3,
        width: "93%",
        height: "93%",
        id: "2-3",
      },
      {
        srcM: Music11M,
        srcH: Music11H,
        colStart: 2,
        colSpan: 4,
        rowStart: 31,
        rowSpan: 7,
        id: "2-4",
      },
      {
        srcM: Music12M,
        srcH: Music12H,
        colStart: 7,
        colSpan: 6,
        rowStart: 33,
        rowSpan: 4,
        id: "3-0",
      },
      {
        srcM: Music13M,
        srcH: Music13H,
        colStart: 5,
        colSpan: 5,
        rowStart: 39,
        rowSpan: 4,
        id: "3-1",
      },
      {
        srcM: Music14M,
        srcH: Music14H,
        colStart: 8,
        colSpan: 4,
        rowStart: 41,
        rowSpan: 3,
        id: "3-2",
      },
      {
        srcM: Music15M,
        srcH: Music15H,
        colStart: 1,
        colSpan: 3,
        rowStart: 42,
        rowSpan: 6,
        id: "3-3",
      },
    ],
  },
  nature: {
    heroImage: {
      mobileSrc: Nature7M,
      src: Nature7H,
    },
    projectImages: [
      {
        srcM: Nature1M,
        srcH: Nature1H,
        colStart: 1,
        colSpan: 4,
        rowStart: 1,
        rowSpan: 5,
        id: "1-0",
      },
      {
        srcM: Nature2M,
        srcH: Nature2H,
        colStart: 6,
        colSpan: 3,
        rowStart: 2,
        rowSpan: 3,
        id: "1-1",
      },
      {
        srcM: Nature3M,
        srcH: Nature3H,
        colStart: 6,
        colSpan: 6,
        rowStart: 5,
        rowSpan: 3,
        id: "1-2",
      },
      {
        srcM: Nature4M,
        srcH: Nature4H,
        colStart: 2,
        colSpan: 4,
        rowStart: 6,
        rowSpan: 5,
        width: "90%",
        id: "1-3",
      },
      {
        srcM: Nature5M,
        srcH: Nature5H,
        colStart: 8,
        colSpan: 4,
        rowStart: 8,
        rowSpan: 5,
        width: "95%",
        id: "1-4",
      },
      {
        srcM: Nature6M,
        srcH: Nature6H,
        colStart: 1,
        colSpan: 3,
        rowStart: 11,
        rowSpan: 3,
        width: "90%",
        id: "2-0",
      },
      {
        srcM: Nature7M,
        srcH: Nature7H,
        colStart: 4,
        colSpan: 4,
        rowStart: 12,
        rowSpan: 4,
        width: "90%",
        id: "2-1",
      },
      {
        srcM: Nature8M,
        srcH: Nature8H,
        colStart: 9,
        colSpan: 4,
        rowStart: 13,
        rowSpan: 5,
        id: "2-2",
      },
      {
        srcM: Nature9M,
        srcH: Nature9H,
        colStart: 1,
        colSpan: 5,
        rowStart: 16,
        rowSpan: 6,
        id: "2-3",
      },
      {
        srcM: Nature10M,
        srcH: Nature10H,
        colStart: 7,
        colSpan: 4,
        rowStart: 18,
        rowSpan: 4,
        id: "2-4",
      },
      {
        srcM: Nature11M,
        srcH: Nature11H,
        colStart: 5,
        colSpan: 5,
        rowStart: 22,
        rowSpan: 3,
        id: "3-0",
      },
      {
        srcM: Nature12M,
        srcH: Nature12H,
        colStart: 10,
        colSpan: 3,
        rowStart: 23,
        rowSpan: 4,
        width: "90%",
        height: "120%",
        id: "3-1",
      },
      {
        srcM: Nature13M,
        srcH: Nature13H,
        colStart: 2,
        colSpan: 3,
        rowStart: 23,
        rowSpan: 3,
        id: "3-2",
      },
      {
        srcM: Nature14M,
        srcH: Nature14H,
        colStart: 4,
        colSpan: 5,
        rowStart: 28,
        rowSpan: 6,
        id: "3-3",
      },
      {
        srcM: Nature15M,
        srcH: Nature15H,
        colStart: 9,
        colSpan: 4,
        rowStart: 29,
        rowSpan: 6,
        id: "3-4",
      },
      {
        srcM: Nature16M,
        srcH: Nature16H,
        colStart: 2,
        colSpan: 4,
        rowStart: 35,
        rowSpan: 6,
        id: "4-0",
      },
      {
        srcM: Nature17M,
        srcH: Nature17H,
        colStart: 7,
        colSpan: 4,
        rowStart: 36,
        rowSpan: 4,
        height: "120%",
        id: "4-1",
      },
      {
        srcM: Nature18M,
        srcH: Nature18H,
        colStart: 1,
        colSpan: 4,
        rowStart: 41,
        rowSpan: 5,
        id: "4-2",
      },
      {
        srcM: Nature19M,
        srcH: Nature19H,
        colStart: 9,
        colSpan: 3,
        rowStart: 41,
        rowSpan: 4,
        height: "115%",
        id: "4-3",
      },
      {
        srcM: Nature20M,
        srcH: Nature20H,
        colStart: 2,
        colSpan: 3,
        rowStart: 46,
        rowSpan: 5,
        id: "4-4",
      },
      {
        srcM: Nature21M,
        srcH: Nature21H,
        colStart: 6,
        colSpan: 4,
        rowStart: 47,
        rowSpan: 6,
        width: "90%",
        id: "5-0",
      },
      {
        srcM: Nature22M,
        srcH: Nature22H,
        colStart: 5,
        colSpan: 3,
        rowStart: 43,
        rowSpan: 4,
        width: "130%",
        height: "115%",
        id: "5-1",
      },
      {
        srcM: Nature23M,
        srcH: Nature23H,
        colStart: 1,
        colSpan: 5,
        rowStart: 52,
        rowSpan: 4,
        width: "90%",
        id: "5-2",
      },
      {
        srcM: Nature24M,
        srcH: Nature24H,
        colStart: 6,
        colSpan: 3,
        rowStart: 54,
        rowSpan: 4,
        id: "5-3",
      },
      {
        srcM: Nature25M,
        srcH: Nature25H,
        colStart: 10,
        colSpan: 3,
        rowStart: 53,
        rowSpan: 6,
        height: "110%",
        id: "5-4",
      },
      {
        srcM: Nature26M,
        srcH: Nature26H,
        colStart: 1,
        colSpan: 6,
        rowStart: 58,
        rowSpan: 7,
        id: "6-0",
      },
      {
        srcM: Nature27M,
        srcH: Nature27H,
        colStart: 9,
        colSpan: 4,
        rowStart: 1,
        rowSpan: 3,
        height: "130%",
        id: "6-1",
      },
      {
        srcM: Nature28M,
        srcH: Nature28H,
        colStart: 9,
        colSpan: 4,
        rowStart: 64,
        rowSpan: 5,
        id: "6-2",
      },
      {
        srcM: Nature29M,
        srcH: Nature29H,
        colStart: 7,
        colSpan: 5,
        rowStart: 59,
        rowSpan: 5,
        id: "6-3",
      },
    ],
  },
  lifestyle: {
    heroImage: {
      mobileSrc: Lifestyle12M,
      src: Lifestyle12H,
    },
    projectImages: [
      {
        srcM: Lifestyle1M,
        srcH: Lifestyle1H,
        colStart: 1,
        colSpan: 4,
        rowStart: 1,
        rowSpan: 8,
        id: "1-0",
      },
      {
        srcM: Lifestyle2M,
        srcH: Lifestyle2H,
        colStart: 7,
        colSpan: 3,
        rowStart: 2,
        rowSpan: 6,
        id: "1-1",
      },
      {
        srcM: Lifestyle3M,
        srcH: Lifestyle3H,
        colStart: 2,
        colSpan: 4,
        rowStart: 11,
        rowSpan: 6,
        width: "90%",
        id: "1-2",
      },
      {
        srcM: Lifestyle4M,
        srcH: Lifestyle4H,
        colStart: 6,
        colSpan: 6,
        rowStart: 9,
        rowSpan: 4,
        id: "1-3",
      },
      {
        srcM: Lifestyle5M,
        srcH: Lifestyle5H,
        colStart: 7,
        colSpan: 5,
        rowStart: 14,
        rowSpan: 4,
        width: "95%",
        id: "1-4",
      },
      {
        srcM: Lifestyle6M,
        srcH: Lifestyle6H,
        colStart: 3,
        colSpan: 3,
        rowStart: 18,
        rowSpan: 5,
        width: "90%",
        id: "2-0",
      },
      {
        srcM: Lifestyle7M,
        srcH: Lifestyle7H,
        colStart: 8,
        colSpan: 5,
        rowStart: 19,
        rowSpan: 8,
        id: "2-1",
      },
      {
        srcM: Lifestyle8M,
        srcH: Lifestyle8H,
        colStart: 1,
        colSpan: 4,
        rowStart: 24,
        rowSpan: 9,
        id: "2-2",
      },
      {
        srcM: Lifestyle9M,
        srcH: Lifestyle9H,
        colStart: 6,
        colSpan: 4,
        rowStart: 28,
        rowSpan: 4,
        id: "2-3",
      },
      {
        srcM: Lifestyle10M,
        srcH: Lifestyle10H,
        colStart: 10,
        colSpan: 3,
        rowStart: 30,
        rowSpan: 4,
        id: "2-4",
      },
      {
        srcM: Lifestyle11M,
        srcH: Lifestyle11H,
        colStart: 2,
        colSpan: 4,
        rowStart: 34,
        rowSpan: 6,
        id: "3-0",
      },
      {
        srcM: Lifestyle12M,
        srcH: Lifestyle12H,
        colStart: 6,
        colSpan: 4,
        rowStart: 37,
        rowSpan: 7,
        id: "3-1",
      },
      {
        srcM: Lifestyle13M,
        srcH: Lifestyle13H,
        colStart: 10,
        colSpan: 4,
        rowStart: 36,
        rowSpan: 5,
        id: "3-2",
      },
      {
        srcM: Lifestyle14M,
        srcH: Lifestyle14H,
        colStart: 7,
        colSpan: 6,
        rowStart: 45,
        rowSpan: 10,
        id: "3-3",
      },
      {
        srcM: Lifestyle15M,
        srcH: Lifestyle15H,
        colStart: 1,
        colSpan: 5,
        rowStart: 42,
        rowSpan: 6,
        id: "3-4",
      },
      {
        srcM: Lifestyle16M,
        srcH: Lifestyle16H,
        colStart: 2,
        colSpan: 5,
        rowStart: 49,
        rowSpan: 7,
        id: "4-0",
      },
      {
        srcM: Lifestyle17M,
        srcH: Lifestyle17H,
        colStart: 3,
        colSpan: 6,
        rowStart: 56,
        rowSpan: 3,
        id: "4-1",
      },
      {
        srcM: Lifestyle18M,
        srcH: Lifestyle18H,
        colStart: 9,
        colSpan: 5,
        rowStart: 55,
        rowSpan: 2,
        id: "4-2",
      },
      {
        srcM: Lifestyle19M,
        srcH: Lifestyle19H,
        colStart: 1,
        colSpan: 4,
        rowStart: 59,
        rowSpan: 3,
        id: "4-3",
      },
      {
        srcM: Lifestyle20M,
        srcH: Lifestyle20H,
        colStart: 11,
        colSpan: 3,
        rowStart: 57,
        rowSpan: 2,
        id: "4-4",
      },
      {
        srcM: Lifestyle21M,
        srcH: Lifestyle21H,
        colStart: 8,
        colSpan: 3,
        rowStart: 59,
        rowSpan: 1,
        id: "5-0",
      },
      {
        srcM: Lifestyle22M,
        srcH: Lifestyle22H,
        colStart: 1,
        colSpan: 3,
        rowStart: 62,
        rowSpan: 3,
        id: "5-1",
      },
      {
        srcM: Lifestyle23M,
        srcH: Lifestyle23H,
        colStart: 4,
        colSpan: 3,
        rowStart: 62,
        rowSpan: 2,
        id: "5-2",
      },
      {
        srcM: Lifestyle24M,
        srcH: Lifestyle24H,
        colStart: 7,
        colSpan: 6,
        rowStart: 60,
        rowSpan: 5,
        id: "5-3",
      },
      {
        srcM: Lifestyle25M,
        srcH: Lifestyle25H,
        colStart: 3,
        colSpan: 4,
        rowStart: 65,
        rowSpan: 1,
        id: "5-4",
      },
      {
        srcM: Lifestyle26M,
        srcH: Lifestyle26H,
        colStart: 8,
        colSpan: 5,
        rowStart: 65,
        rowSpan: 2,
        id: "6-0",
      },
      {
        srcM: Lifestyle27M,
        srcH: Lifestyle27H,
        colStart: 1,
        colSpan: 7,
        rowStart: 66,
        rowSpan: 4,
        id: "6-1",
      },
      {
        srcM: Lifestyle29M,
        srcH: Lifestyle29H,
        colStart: 9,
        colSpan: 3,
        rowStart: 67,
        rowSpan: 1,
        id: "6-2",
      },
      // {
      //   src: Lifestyle29,
      //   colStart: 10,
      //   colSpan: 4,
      //   rowStart: 68,
      //   rowSpan: 2,
      //   id: "6-3",
      // },
      {
        srcM: Lifestyle30M,
        srcH: Lifestyle30H,
        colStart: 10,
        colSpan: 4,
        rowStart: 68,
        rowSpan: 2,
        id: "6-3",
      },
    ],
  },
  sports: {
    heroImage: {
      mobileSrc: Sports7M,
      src: Sports7H,
    },
    projectImages: [
      {
        srcM: Sports1M,
        srcH: Sports1H,
        colStart: 1,
        colSpan: 4,
        rowStart: 1,
        rowSpan: 8,
        id: "1-0",
      },
      {
        srcM: Sports2M,
        srcH: Sports2H,
        colStart: 7,
        colSpan: 3,
        rowStart: 2,
        rowSpan: 6,
        id: "1-1",
      },
      {
        srcM: Sports3M,
        srcH: Sports3H,
        colStart: 2,
        colSpan: 4,
        rowStart: 11,
        rowSpan: 6,
        width: "90%",
        id: "1-2",
      },
      {
        srcM: Sports4M,
        srcH: Sports4H,
        colStart: 6,
        colSpan: 6,
        rowStart: 9,
        rowSpan: 4,
        id: "1-3",
      },
      {
        srcM: Sports5M,
        srcH: Sports5H,
        colStart: 7,
        colSpan: 5,
        rowStart: 14,
        rowSpan: 4,
        width: "95%",
        id: "1-4",
      },
      {
        srcM: Sports6M,
        srcH: Sports6H,
        colStart: 3,
        colSpan: 3,
        rowStart: 18,
        rowSpan: 5,
        width: "90%",
        id: "2-0",
      },
      {
        srcM: Sports7M,
        srcH: Sports7H,
        colStart: 8,
        colSpan: 5,
        rowStart: 19,
        rowSpan: 8,
        id: "2-1",
      },
      {
        srcM: Sports8M,
        srcH: Sports8H,
        colStart: 1,
        colSpan: 4,
        rowStart: 24,
        rowSpan: 9,
        id: "2-2",
      },
      {
        srcM: Sports9M,
        srcH: Sports9H,
        colStart: 6,
        colSpan: 4,
        rowStart: 27,
        rowSpan: 6,
        id: "2-3",
      },
      {
        srcM: Sports10M,
        srcH: Sports10H,
        colStart: 9,
        colSpan: 4,
        rowStart: 30,
        rowSpan: 4,
        id: "2-4",
      },
      {
        srcM: Sports11M,
        srcH: Sports11H,
        colStart: 2,
        colSpan: 4,
        rowStart: 33,
        rowSpan: 3,
        id: "2-4",
      },
      {
        srcM: Sports12M,
        srcH: Sports12H,
        colStart: 8,
        colSpan: 3,
        rowStart: 35,
        rowSpan: 1,
        id: "2-4",
      },
      {
        srcM: Sports13M,
        srcH: Sports13H,
        colStart: 1,
        colSpan: 3,
        rowStart: 36,
        rowSpan: 2,
        id: "2-4",
      },
      {
        srcM: Sports14M,
        srcH: Sports14H,
        colStart: 6,
        colSpan: 7,
        rowStart: 36,
        rowSpan: 5,
        id: "2-4",
      },
      {
        srcM: Sports15M,
        srcH: Sports15H,
        colStart: 1,
        colSpan: 5,
        rowStart: 39,
        rowSpan: 1,
        id: "2-4",
      },
      {
        srcM: Sports16M,
        srcH: Sports16H,
        colStart: 9,
        colSpan: 4,
        rowStart: 43,
        rowSpan: 1,
        id: "2-4",
      },
      {
        srcM: Sports17M,
        srcH: Sports17H,
        colStart: 5,
        colSpan: 4,
        rowStart: 41,
        rowSpan: 4,
        id: "2-4",
      },
      {
        srcM: Sports18M,
        srcH: Sports18H,
        colStart: 1,
        colSpan: 3,
        rowStart: 41,
        rowSpan: 2,
        id: "2-4",
      },
    ],
  },
};

export const textAboutImages = {
  experiences: {
    positions: [
      { x: -150, y: -100, scale: 1.2, rotation: -10 },
      { x: 100, y: -150, scale: 1.3, rotation: 15 },
      { x: -120, y: 100, scale: 1.4, rotation: -20 },
      { x: 130, y: 80, scale: 1.1, rotation: 10 },
    ],
    images: [Lifestyle12M, Lifestyle15M, Lifestyle3M, Lifestyle1M],
  },

  websites: {
    positions: [
      { x: -180, y: -130, scale: 1.3, rotation: -5 },
      { x: 120, y: -120, scale: 1.5, rotation: 20 },
      { x: -100, y: 130, scale: 1.2, rotation: -15 },
      { x: 150, y: 100, scale: 1.0, rotation: 5 },
    ],
    images: [Photoshoots13M, Photoshoots1M, Photoshoots3M, Photoshoots16M],
  },
  visions: {
    positions: [
      { x: -160, y: -140, scale: 1.1, rotation: -8 },
      { x: 100, y: -140, scale: 1.4, rotation: 18 },
      { x: -100, y: 100, scale: 1.2, rotation: -12 },
      { x: 110, y: 100, scale: 1.1, rotation: 8 },
    ],
    images: [Lifestyle2M, Lifestyle13M, Lifestyle24M, Nature16M],
  },
};

export const favoritesAbout = [
  {
    src: Lifestyle1M,
    name: '"Echoes of Elegance"',
  },
  {
    src: NatureM,
    name: '"Silent Watcher"',
  },
  {
    src: Photoshoots31M,
    name: '"Gravity in Motion"',
  },
  {
    src: Nature8M,
    name: '"Roadside Bond"',
  },
  {
    src: Sports9M,
    name: '"Chasing Liquid Mountains"',
  },
  {
    src: Lifestyle13M,
    name: '"Timeless Pause"',
  },
];

//MOBILE

export const mobilePortfolioCard = [
  {
    src: PhotoshootsPortfolio,
    title: "portfolio.cards.photoshoots.title",
    subtitle: "2023",
    url: "/portfolio/photoshoots",
  },
  {
    src: Nature15M,
    title: "portfolio.cards.nature.title",
    subtitle: "2024",
    url: "/portfolio/nature",
  },
  {
    src: Sports4M,
    title: "portfolio.cards.sports.title",
    subtitle: "2023",
    url: "/portfolio/sports",
  },
  {
    src: Music3M,
    title: "portfolio.cards.music.title",
    subtitle: "2023",
    url: "/portfolio/music",
  },
  {
    src: Lifestyle18M,
    title: "portfolio.cards.lifestyle.title",
    subtitle: "2024",
    url: "/portfolio/lifestyle",
  },
];

export const editorialsCoversMbl = [
  {
    src: EditorialCover1Mbl,
    name: "GREECE",
    date: "0723",
    url: "/editorials/greece",
  },
  {
    src: EditorialCover2Mbl,
    name: "TANZANIA",
    date: "0423",
    url: "/editorials/tanzania",
  },
  {
    src: EditorialCover3Mbl,
    name: "DOLOMITES",
    date: "0624",
    url: "/editorials/dolomites",
  },
];

export const contactLinks = [
  {
    text: "LINKEDIN",
    logoClass: "icon-linkedin",
    href: "https://www.linkedin.com/in/paula-sanz-perez/",
  },
  {
    text: "GITHUB",
    logoClass: "icon-github",
    href: "https://github.com/paussaanz",
  },
  {
    text: "INSTAGRAM",
    logoClass: "icon-instagram",
    href: "https://www.instagram.com/sypcreative/",
  },
  {
    text: "BEHANCE",
    logoClass: "icon-behance",
    href: "https://www.behance.net/paulasanz1",
    logoSize: "h6",
  },
];
